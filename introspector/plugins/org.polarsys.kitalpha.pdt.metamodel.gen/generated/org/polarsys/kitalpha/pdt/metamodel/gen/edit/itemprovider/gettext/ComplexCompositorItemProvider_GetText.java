...
public class ComplexCompositorItemProvider_GetText
		extends org.eclipse.egf.emf.pattern.edit.call.ItemProvider.ItemProvidergetTextoverride {
	protected static String nl;

	public static synchronized ComplexCompositorItemProvider_GetText create(String lineSeparator) {
		nl = lineSeparator;
		ComplexCompositorItemProvider_GetText result = new ComplexCompositorItemProvider_GetText();
		nl = null;
		return result;
	}
	...
	protected final String TEXT_1 = "\t\t/**" + NL + "\t\t * CUSTOM_CODE_BEGIN : []" + NL + "\t\t **/" + NL
			+ "\t\tComplexCompositor complexCompositor = (ComplexCompositor) object;" + NL + "\t\tString label = \"\";"
			+ NL + "\t\tint complexCompCard = complexCompositor.getMaxCard();" + NL + "" + NL
			+ "\t\tif (complexCompositor.getComplexCompositorKind().equals(" + NL
			+ "\t\t\t\tComplexCompositorKind.SEQUENCE)) {" + NL + "\t\t\tif (complexCompositor.isUnbounded())" + NL
			+ "\t\t\t\tlabel = \"Sequence [\" + complexCompositor.getMinCard() + \";*]\";" + NL + "\t\t\telse" + NL
			+ "\t\t\t\tlabel = \"Sequence [\" + complexCompositor.getMinCard() + \";\"" + NL
			+ "\t\t\t\t\t\t+ complexCompCard + \"]\";" + NL + "" + NL + "\t\t} else {" + NL
			+ "\t\t\tif (complexCompositor.isUnbounded())" + NL
			+ "\t\t\t\tlabel = \"Choice [\" + complexCompositor.getMinCard() + \";*]\";" + NL + "\t\t\telse" + NL
			+ "\t\t\t\tlabel = \"Choice [\" + complexCompositor.getMinCard() + \";\"" + NL
			+ "\t\t\t\t\t\t+ complexCompCard + \"]\";" + NL + "\t\t}" + NL + "\t\treturn label;" + NL + "\t\t/**" + NL
			+ "\t\t * CUSTOM_CODE_END : []" + NL + "\t\t **/" + NL;
	protected final String TEXT_2 = NL;

	public ComplexCompositorItemProvider_GetText() {
